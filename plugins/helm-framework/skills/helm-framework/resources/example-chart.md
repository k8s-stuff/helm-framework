...
# Rollout and shutdown timing. All three are omitted from the rendered
# Deployment while unset, leaving the Kubernetes defaults (0 / 30 / no hooks).
# minReadySeconds: 30
...
jobs:
  - name: migrate
    ...
    backoffLimit: 6
    # Map form of envVars, exercised here so the default render covers both
    # accepted shapes at once (the top-level key below stays in list form).
    envVars:
      JOB_MODE: migrate
      JOB_ATTEMPT: 1
...
sidecars:
  - name: proxy
    ...
    # Written in map form against a list-form release-wide envVars, so the render
    # covers helm-framework.sidecar.env merging the two shapes together.
    envVars:
      foo: sidecar-bar
